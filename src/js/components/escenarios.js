...
const createImages = (dataImages) => {
  const arrayImages = dataImages.map((element, number) => {
    ...
    const containerImages = newContainer(
      'div',
      [textEscenarios, imagesEscenarios],
      ['escenarios__container-images', `escenarios__images--${number}`],
    );
...
const escenariosComp = (data) => {
  ...
  const containerPrincipal = newContainer(
    'div',
    [...imagesEscenarios],
    ['escenarios__principal-container'],
  );
